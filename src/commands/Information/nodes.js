...
module.exports = class Nodes extends Command {
  ...
  async execute({ message, args }) {
    ...
    const lavalinkEU2Ping = await this.client.music.nodes.find(n => n.identifier === 'Black Hole').ping();
    ...
    embed.addFields([
      ...
      },
      {
        ...
        value: `> ${e.Switch} | Status: **${String(nodes[3].state).replace("0", "Reconectando").replace("1", "Conectado").replace("2", "Desconectado")}**
        > ${e.Earth} | Região: **${nodes[3].options.region}**
        > ${e.MusicBox} | Players: **${nodes[3].stats.players}**
        > ${e.Time} | Uptime: **${moment
